...
const ru = {
  ...
  'diag.elbow.summary.ok': 'макс. излом {max}° · норма (< {ok}°)',
  'diag.elbow.summary.warn': 'макс. излом {max}° · предупреждение · {seg}',
  'diag.elbow.summary.fail': 'макс. излом {max}° · дефект · {seg}',
  'diag.elbow.detail': 'Худший дискретный угол поворота на плечах: {max}° на сегменте {seg} (обход {round}), доля пути {frac} (~{pct}%). Пороги: норма < {ok}°, предупреждение {ok}–{fail}°, дефект ≥ {fail}°. Форма плеча: {form}.',
...
const en = {
  ...
  'diag.elbow.summary.ok': 'max turn {max}° · ok (< {ok}°)',
  'diag.elbow.summary.warn': 'max turn {max}° · kink warn · {seg}',
  'diag.elbow.summary.fail': 'max turn {max}° · kink fail · {seg}',
  'diag.elbow.detail': 'Worst discrete turn on legs: {max}° at seg {seg} (round {round}), frac {frac} (~{pct}% along leg). Thresholds: ok < {ok}°, warn {ok}–{fail}°, fail ≥ {fail}°. shoulderForm={form}.',
