...
export const PARAM_SCHEMA = [
  ...
  { key: 'hw', group: 'thread', label: 'Cross-section h/w', type: 'number', def: 0.65, min: 0.2, max: 1, step: 0.01,
    ...
    used: 'lift mechanics h = hw·w (#5, liftMode ideal / measured); diagnostics “length along thread axis”' },
  ...
  { key: 'tension_N', group: 'process', label: 'Tension T, N', type: 'number', def: '', min: 0, max: 20, step: 0.1, optional: true,
    basis: 'not measured (uncertainties P3); Olympus: «糸を少し緩ませて»; empty → 1 N assumed (lift-spec v1 §2 (d))', status: 'stored', used: 'lift mechanics (#5): t_c, δ, s₀, λ_T; not used by geometry' },
  { key: 'liftMode', group: 'process', label: 'Lift at crossings', type: 'select', def: 'ideal', options: ['display', 'ideal', 'measured'],
    optionLabels: { display: 'display (the former 0.6·w tent, display only; lengths are a lower bound)', ideal: 'ideal (mechanics, lift-spec v1 T7–T10 at the §2 estimates)', measured: 'measured (Δ₁ from a photo, lift1_w; the rest as ideal)' },
    basis: '#5 lift-spec v1 §5.4 (model/lift-spec.md); default ideal (mechanics); display keeps the former tent for comparison', status: 'intent', used: 'render lift, lengths table, V14, V25–V27' },
  { key: 'wrapK_Nmm2', group: 'process', label: 'Wrap stiffness k, N/mm²', type: 'number', def: 3, min: 0.3, max: 10, step: 0.1,
    basis: 'lift-spec v1 §2 (c): wrap modulus 2–5 MPa, spot 0.7×2 mm → k ≈ 2–5; range 0.3–10', status: 'estimate', used: 'lift mechanics: δ, s₀, λ_T, Π' },
  { key: 'bendB_Nmm2', group: 'process', label: 'Thread bending stiffness B, N·mm²', type: 'number', def: 0.0155, min: 0.003, max: 0.1, step: 0.0005,
    basis: 'lift-spec v1.1 §2 (b): measured on an analogue thread — Alshukur & Macintyre 2020 (cotton 3-ply 126 tex, 1.55e−8 N·m², CV 49 %); range 0.01–0.03', status: 'source', used: 'lift mechanics: Π, ℓ_b, δ (5–15 %)' },
  { key: 'stackKappa', group: 'process', label: 'Stack factor κ', type: 'number', def: 0.8, min: 0.7, max: 1, step: 0.01,
    basis: 'lift-spec v1 §1.6 (b): Durur t₆/(3t₂) 0.82–1.02, Panneerselvam 2024; range 0.7–1.0', status: 'estimate', used: 'lift mechanics: Δ(m) = Δ₁·[1 + κ(m − 1)]' },
  { key: 'lift1_w', group: 'process', label: 'Single-crossing lift Δ₁ from a photo, × w', type: 'number', def: '', min: 0, max: 1.5, step: 0.01, optional: true,
    basis: 'lift-spec v1 §5.1–5.2: bump height on the limb in a side-lit photo; empty → model value (ideal)', status: 'stored', used: 'if liftMode = measured' },
